...
- Contract (when `CompanionState` lands): taps call `CompanionState.care("charge"|"wipe"|"spray"|"repair")`; UI listens to `stats_changed` / `leveled_up` / `tidbit_unlocked`; HUD strip (`ui/hud_strip.tscn`, Phase 2) reads the same signals from every screen.
